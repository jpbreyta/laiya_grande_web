        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
                ...
                <div class="relative p-8 md:p-10">
                    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                        <div class="space-y-3">
                            ...
                            <div class="space-y-2">
                                <h1 class="text-3xl md:text-4xl font-black tracking-tight">Edit Room</h1>
                                <p class="max-w-xl text-emerald-50 text-sm md:text-base leading-relaxed">
                                    Update room information, amenities, and availability to reflect the latest offerings.
                                </p>
                            </div>
                        </div>
            ...
            <div class="rounded-2xl bg-white shadow-xl ring-1 ring-slate-200">
                <div class="px-6 py-8 md:px-8">
                    <form action="{{ route('admin.room.update', $room) }}" method="POST" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea id="short_description" name="short_description" rows="3"
                                    class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300">{{ old('short_description', $room->short_description) }}</textarea>
                            </div>
                            <div>
                                ...
                                <textarea id="full_description" name="full_description" rows="3"
                                    class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300">{{ old('full_description', $room->full_description) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="image" name="image" value="{{ old('image', $room->image) }}"
                                    class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300">
                            </div>
                            <div>
                                <label for="rate_name" class="block text-sm font-semibold text-slate-700">Rate Name</label>
                                <input type="text" id="rate_name" name="rate_name" value="{{ old('rate_name', $room->rate_name) }}"
                                    class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300">
                            </div>
                            <div>
                                <label for="status" class="block text-sm font-semibold text-slate-700">Status</label>
                                <input type="text" id="status" name="status" value="{{ old('status', $room->status) }}" required
                                    class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300">
                            </div>
                        </div>
                                ...
                                @foreach ($featureFields as $field)
                                    <label for="{{ $field['id'] }}" class="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 shadow-sm border border-slate-200">
                                        <input type="checkbox" id="{{ $field['id'] }}" name="{{ $field['id'] }}" value="1"
                                            {{ old($field['id'], $room->{$field['id']}) ? 'checked' : '' }}
