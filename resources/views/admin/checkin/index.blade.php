...
<section class="p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    <div class="max-w-7xl mx-auto">
        <h1 class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-800 to-gray-600 mb-2">Checked-in Guests</h1>
        <p class="text-gray-600 mb-6">Manage and monitor all checked-in guests. View details and manage guest status.</p>
                            ...
                            @foreach ($checkedInBookings as $booking)
                                <tr class="hover:bg-gradient-to-r hover:from-blue-50/30 hover:to-indigo-50/20 transition-all duration-200">
                                    ...
                                    <td class="py-3 px-4">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold shadow-sm {{ $statusStyles[$booking->status] ?? 'bg-gray-100 text-gray-800 border border-gray-200' }}">
                                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $booking->status === 'active' ? 'bg-green-500' : ($booking->status === 'completed' ? 'bg-blue-500' : 'bg-gray-500') }}"></span>
