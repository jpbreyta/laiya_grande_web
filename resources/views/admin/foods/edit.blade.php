        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
                ...
                <div class="relative p-8 md:p-10">
                    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                        <div class="space-y-3">
                            ...
                            <div class="space-y-2">
                                <h1 class="text-3xl md:text-4xl font-black tracking-tight">Edit Food Item</h1>
                                <p class="max-w-xl text-emerald-50 text-sm md:text-base leading-relaxed">
                                    Update menu details to ensure pricing and descriptions stay accurate across the platform.
                                </p>
                            </div>
                        </div>
            ...
            <div class="rounded-2xl bg-white shadow-xl ring-1 ring-slate-200">
                <div class="px-6 py-8 md:px-8">
                    <form action="{{ route('admin.foods.update', $food->id) }}" method="POST" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="mt-2 relative">
                                    ...
                                    <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $food->price) }}" required
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300">{{ old('description', $food->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-xs font-semibold text-rose-500">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div>
                                ...
                                <select name="food_category_id" id="food_category_id" required
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" {{ old('food_category_id', $food->food_category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
