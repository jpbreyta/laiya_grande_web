    ...
    <section class="p-6 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
        <div class="max-w-7xl mx-auto">
            ...
            @include('admin.components.table-controls', [
                ...
                'button' => [
                    'type' => 'deleted',
                    'text' => 'Deleted Reservations',
                    'route' => 'admin.reservation.deleted',
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($reservations as $reservation)
                                <tr class="hover:bg-gradient-to-r hover:from-blue-50/30 hover:to-indigo-50/20 transition-all duration-200"
                                    ...
                                    <td class="py-3 px-4 text-gray-700 font-medium">{{ $reservation->reservation_number }}
                                    </td>
                                    <td class="py-3 px-4 font-medium text-gray-900">{{ $reservation->firstname }}
                                    ...
                                    <td
                                        ...
                                        ₱{{ number_format($reservation->total_price, 2) }}</td>
                                    ...
                                    <td class="py-3 px-4">
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold shadow-sm {{ $statusStyles[$reservation->status] ?? 'bg-gray-100 text-gray-800' }}">
                                            <span
                                                class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $reservation->status === 'confirmed' ? 'bg-green-500' : ($reservation->status === 'pending' ? 'bg-amber-500' : ($reservation->status === 'paid' ? 'bg-blue-500' : 'bg-red-500')) }}"></span>
