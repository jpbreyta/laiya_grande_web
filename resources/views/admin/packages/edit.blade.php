        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
                ...
                <div class="relative p-8 md:p-10">
                    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                        <div class="space-y-3">
                            ...
                            <div class="space-y-2">
                                <h1 class="text-3xl md:text-4xl font-black tracking-tight">Edit Package</h1>
                                <p class="max-w-xl text-emerald-50 text-sm md:text-base leading-relaxed">
                                    Update package details, pricing, and descriptions to keep your offerings current and attractive.
                                </p>
                            </div>
                        </div>
            ...
            <div class="rounded-2xl bg-white shadow-xl ring-1 ring-slate-200">
                <div class="px-6 py-8 md:px-8">
                    <form action="{{ route('admin.packages.update', $package->id) }}" method="POST" class="space-y-8" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="type" name="type"
                                        ...
                                        <option value="Day Tour" {{ old('type', $package->type ?? '') == 'Day Tour' ? 'selected' : '' }}>Day Tour</option>
                                        <option value="Overnight" {{ old('type', $package->type ?? '') == 'Overnight' ? 'selected' : '' }}>Overnight</option>
                                        <option value="Weekend" {{ old('type', $package->type ?? '') == 'Weekend' ? 'selected' : '' }}>Weekend</option>
                                        <option value="Week Package" {{ old('type', $package->type ?? '') == 'Week Package' ? 'selected' : '' }}>Week Package</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" id="capacity" name="capacity" value="{{ old('capacity', $package->capacity ?? '') }}"
                                        placeholder="e.g., 4"
                                        class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm transition focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" id="image_path" name="image_path" value="{{ old('image_path', $package->image_path) }}"
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="6" required
                                    ...
                                    class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm transition focus:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-300 resize-none">{{ old('description', $package->description) }}</textarea>
                                @error('description')
                                    <p class="mt-2 text-xs font-semibold text-rose-500 flex items-center gap-1">
                                        <i class="fas fa-exclamation-circle"></i>
                                        {{ $message }}
