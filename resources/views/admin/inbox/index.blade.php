...
<div class="poppins min-h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-gray-200 px-6 py-10">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
        <div class="flex items-start justify-between mb-8">
            <div>
                <h1 class="text-2xl md:text-3xl font-semibold text-gray-900 tracking-tight">📥 Admin Inbox</h1>
            ...
            <div class="flex gap-2 items-center">
                ...
                <button id="deleteBtn"
                    ...
                    <span class="absolute -bottom-9 left-1/2 -translate-x-1/2 
        ...
        <div class="flex flex-wrap gap-3 mb-6 text-sm font-medium">
            <button class="px-4 py-1.5 rounded-full bg-gray-100 text-gray-700 hover:bg-teal-100 hover:text-teal-700 transition">All</button>
            <button class="px-4 py-1.5 rounded-full bg-gray-100 text-gray-700 hover:bg-teal-100 hover:text-teal-700 transition">Read</button>
            <button class="px-4 py-1.5 rounded-full bg-gray-100 text-gray-700 hover:bg-teal-100 hover:text-teal-700 transition">Unread</button>
        </div>

        @if ($messages->count() > 0)
            <ul class="divide-y divide-gray-100">
                @foreach ($messages as $message)
                    <li class="mail-item group p-5 flex items-start gap-4 rounded-lg transition {{ $message->status === 'unread' ? 'bg-blue-50' : 'hover:bg-gray-50' }}">
                        <input type="checkbox"
                               class="mt-2 rounded border-gray-300 focus:ring-0 message-checkbox"
                               value="{{ $message->id }}" />
                        ...
                        <div class="flex-1 cursor-pointer" onclick="window.location.href='{{ route('admin.inbox.show', $message->id) }}'">
                            <div class="flex items-center gap-2">
                                <div class="font-medium text-gray-900">{{ $message->name }}</div>
                                <div class="text-xs text-gray-500 italic">&lt;{{ $message->email }}&gt;</div>
                                <div class="ml-auto text-xs text-gray-400">{{ $message->created_at->format('M j') }}</div>
                            </div>
                            <div class="text-sm text-gray-700 mt-1 font-medium">{{ Str::limit($message->subject, 50) }}</div>
