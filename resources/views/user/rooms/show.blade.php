    ...
    <div class="bg-slate-50 min-h-screen font-body text-slate-600 pb-20">
        ...
        <div class="max-w-7xl mx-auto px-6 pt-8 pb-6">
            <a href="{{ route('user.rooms.index') }}"
                ...
                <div
                    class="w-8 h-8 rounded-full bg-white border border-slate-200 flex items-center justify-center mr-3 group-hover:border-teal-500 shadow-sm">
        ...
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2 space-y-10">
                ...
                <div
                    class="relative group rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50 bg-slate-200 h-[400px] md:h-[500px]">
                    <div id="slider" class="slider-track h-full">
                        @if (!empty($room->images) && count($room->images) > 0)
                            ...
                            @endforeach
                        @else
                            <img src="{{ asset($room->image ?? 'images/default-room.jpg') }}"
                    ...
                    @if (!empty($room->images) && count($room->images) > 1)
                        <div
                            ...
                            <button onclick="slide(-1)"
                                class="pointer-events-auto w-12 h-12 rounded-full bg-white/90 backdrop-blur-sm text-slate-800 hover:text-teal-700 shadow-lg flex items-center justify-center hover:scale-110 transition-all">
                        ...
                        <div class="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2">
                            @foreach ($room->images as $index => $img)
                                <div class="w-2 h-2 rounded-full bg-white/50 dot-indicator"
                ...
                <div>
                    <div class="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
                        <div>
                            ...
                            <div class="flex items-center gap-4 text-sm text-slate-500">
                                <span class="flex items-center gap-1"><i class="fas fa-star text-yellow-400"></i> 5.0
                                    (Reviews)</span>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                            <div
                                class="w-10 h-10 rounded-full bg-white text-teal-600 flex items-center justify-center shadow-sm">
                        ...
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                            ...
                            <div>
                                ...
                                <p class="text-slate-800 font-medium">
                                    {{ $room->has_private_cr ? 'Private En-suite' : 'Shared' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                            ...
                            <div>
                                ...
                                <p class="text-slate-800 font-medium">
                                    {{ $room->has_aircon ? 'Air Conditioned' : 'Fan Cooled' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                            ...
                            <div>
                                ...
                                <p class="text-slate-800 font-medium">{{ $room->has_free_parking ? 'Free Slot' : 'Paid' }}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-1">
                <div
                    class="sticky top-8 bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-100 p-6 lg:p-8">
                    ...
                    <div class="flex justify-between items-end mb-6">
                        <div>
                            ...
                            <div class="flex items-baseline gap-1">
                                <span
                                    class="text-3xl font-heading font-bold text-slate-900">₱{{ number_format($room->price, 0) }}</span>
                            </div>
                        </div>
