    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div id="nav-inner" class="flex justify-between items-center h-16 transition-all duration-500">
            ...
            <div class="hidden xl:flex items-center gap-8 text-sm font-medium text-slate-500 mx-auto whitespace-nowrap">
                ...
                <a href="{{ url('/rooms') }}" class="hover:text-teal-700 transition-colors relative group">Rooms <span
                        class="absolute -bottom-1 left-0 w-0 h-0.5 bg-teal-600 transition-all duration-300 group-hover:w-full"></span></a>
                <a href="{{ url('/gallery') }}" class="hover:text-teal-700 transition-colors relative group">Gallery
                    <span
                        class="absolute -bottom-1 left-0 w-0 h-0.5 bg-teal-600 transition-all duration-300 group-hover:w-full"></span></a>
                <a href="{{ route('location') }}" class="hover:text-teal-700 transition-colors relative group">About Us
                    <span
                        class="absolute -bottom-1 left-0 w-0 h-0.5 bg-teal-600 transition-all duration-300 group-hover:w-full"></span></a>
            ...
            <div class="hidden lg:flex items-center gap-6 flex-shrink-0">
                <div class="flex items-center gap-4 text-slate-400">
                    <a href="#" class="hover:text-teal-600 transition-transform hover:-translate-y-0.5"><i
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div x-data="cartDropdown()" x-init="initCart()" @cart-updated.window="fetchCart()"
                        ...
                        <button @click="open = !open" @click.outside="open = false"
                            class="relative group p-1 text-slate-500 hover:text-teal-600 transition-colors flex items-center outline-none">
                            ...
                            <span x-show="totalCount > 0" x-text="totalCount" x-transition.scale
                                class="absolute -top-2 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-teal-600 text-[10px] font-bold text-white shadow-md border border-white"></span>
                        </button>
                        ...
                        <div x-show="open" x-cloak
                            ...
                            <div
                                class="absolute -top-[6px] right-[18px] w-3 h-3 bg-white transform rotate-45 border-l border-t border-slate-100 z-50">
                            </div>

                            <div
                                class="relative bg-white border border-slate-100 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] overflow-hidden font-poppins">

                                <div
                                    class="px-5 py-3 bg-white border-b border-slate-50 flex justify-between items-center relative z-10">
                                    <div class="flex flex-col">
                                        <h4 class="font-playfair text-slate-800 font-bold text-base tracking-tight">Your
                                            Selection</h4>
                                    ...
                                    <button @click="open = false"
                                        class="text-slate-300 hover:text-slate-500 transition-colors p-1">
                                        <i class="fa-solid fa-xmark text-sm"></i>
                                    </button>
                                </div>

                                <div
                                    class="max-h-[50vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent bg-white">
                                    ...
                                    <template x-for="(item, id) in cartItems" :key="id">
                                        <div
                                            class="relative p-3 border-b border-slate-50 hover:bg-slate-50 transition-colors duration-200">

                                            <div class="flex gap-3">
                                                <div
                                                    class="relative w-14 h-14 rounded-md overflow-hidden flex-shrink-0 bg-slate-100 border border-slate-100">
                                                ...
                                                <div class="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                                                    <div class="flex justify-between items-start">
                                                        <h5 class="font-playfair font-bold text-slate-800 text-sm leading-tight truncate w-32"
                                                            x-text="item.room_name"></h5>

                                                        <button @click="removeFromCart(id)"
                                                            class="text-slate-300 hover:text-rose-500 transition-colors text-xs"
                                                            title="Remove">
                                                    ...
                                                    <div class="flex justify-between items-end mt-1">
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            <span
                                                                class="text-[9px] font-bold text-green-600 bg-green-50 px-1 rounded uppercase tracking-wider">
                                                                Instant Confirm
                                                            </span>
                                                        </div>
                                    ...
                                    <div class="flex justify-between items-end mb-3">
                                        <div>
                                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                                Total</p>
                                            <p class="text-[9px] text-green-600 font-medium leading-none mt-0.5">Incl.
                                                taxes & fees</p>
                                        </div>
                                        <span class="text-xl font-bold text-slate-800 font-playfair"
                                            x-text="'₱' + Number(totalPrice).toLocaleString()"></span>
                                    </div>

                                    <div class="grid grid-cols-2 gap-2">
                                        <a href="{{ route('cart.index') }}"
                                            class="flex justify-center items-center py-2 rounded-lg border border-slate-200 bg-white text-slate-600 text-[11px] font-bold hover:bg-slate-50 hover:text-slate-900 transition-all">
                                            View Cart
                                        </a>
                                        <a href="{{ route('cart.index') }}"
                                            class="flex justify-center items-center py-2 rounded-lg bg-teal-700 text-white text-[11px] font-bold hover:bg-teal-800 transition-all shadow-md shadow-teal-700/20 active:scale-[0.98]">
                                            Book Now
                                        </a>
                                    </div>
                                </div>
                    ...
                    <a href="{{ route('user.rooms.index') }}"
                        class="bg-[#2f4f4f] hover:bg-[#253e3e] text-white px-5 py-2 rounded-full font-medium text-xs transition-colors shadow-lg shadow-teal-900/10 tracking-wide">
                        Book Now
                    </a>
                </div>
            </div>
