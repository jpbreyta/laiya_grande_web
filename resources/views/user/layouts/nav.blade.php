        ...
        <div id="nav-inner" class="flex justify-between items-center h-16 transition-all duration-500">
            ...
            <div class="hidden xl:flex items-center gap-8 text-sm font-medium text-slate-500 mx-auto whitespace-nowrap">
                <a href="{{ url('/') }}" class="hover:text-teal-700 transition-colors relative group">
                    Home
                    <span
                        class="absolute -bottom-1 left-0 w-0 h-0.5 bg-teal-600 transition-all duration-300 group-hover:w-full"></span>
                </a>
            ...
            <div class="hidden lg:flex items-center gap-6 flex-shrink-0">
                ...
                <div class="flex items-center gap-4 text-slate-400">
                    <a href="https://www.facebook.com/laiyagrande" target="_blank"
                        class="hover:text-teal-600 transition-transform hover:-translate-y-0.5"><i
                            class="fa-brands fa-facebook text-lg"></i></a>
                    <a href="https://www.instagram.com/laiyagrande" target="_blank"
                        class="hover:text-teal-600 transition-transform hover:-translate-y-0.5"><i
                            class="fa-brands fa-instagram text-lg"></i></a>
                    <a href="https://www.tiktok.com/@laiyagrandebeachresort" target="_blank"
                        class="hover:text-teal-600 transition-transform hover:-translate-y-0.5"><i
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div x-data="cartDropdown()" x-init="initCart()" @cart-updated.window="fetchCart()"
                        class="relative z-50">

                        <button @click="open = !open" @click.outside="open = false"
                            class="relative group p-1 text-slate-500 hover:text-teal-600 transition-colors flex items-center">
                            <i class="fa-solid fa-cart-shopping text-xl"></i>
                            <span x-show="totalCount > 0" x-text="totalCount" x-transition.scale
                                class="absolute -top-1.5 -right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-yellow-400 text-[10px] font-bold text-teal-900 border border-white">
                            </span>
                        </button>
                        ...
                        <div x-show="open" x-cloak
                            ...
                            <div
                                class="absolute -top-2 right-[6px] w-4 h-4 bg-white transform rotate-45 border-l border-t border-slate-200 z-50">
                            </div>

                            <div
                                class="relative bg-white border border-slate-200 rounded-2xl shadow-2xl shadow-slate-300/50 z-40">

                                <div
                                    class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white rounded-t-2xl relative z-10">
                                    <div>
                                        <h4 class="text-slate-900 font-bold text-lg tracking-tight">Your Stay</h4>
                                        <p class="text-xs text-slate-400 font-medium">
                                            <span x-text="totalCount"></span> Room(s) Selected
                                        </p>
                                    </div>
                                    <button @click="open = false"
                                        class="p-2 rounded-full hover:bg-slate-50 text-slate-300 hover:text-slate-500 transition-colors">
                                        <i class="fa-solid fa-xmark text-lg"></i>
                                    </button>
                                </div>

                                <div
                                    class="max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                                    ...
                                    <template x-for="(item, id) in cartItems" :key="id">
                                        <div
                                            class="group relative flex gap-4 p-5 hover:bg-slate-50 transition-colors duration-200 border-b border-slate-50 last:border-0">
                                            <div
                                                class="relative w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 border border-slate-100 shadow-sm bg-slate-100 flex items-center justify-center">
                                                <i class="fa-solid fa-bed text-slate-300 text-xl"></i>
                                            </div>

                                            <div class="flex-1 min-w-0 flex flex-col justify-between">
                                                <div>
                                                    <div class="flex justify-between items-start">
                                                        <h5 class="text-sm font-bold text-slate-800 leading-tight truncate pr-2"
                                                            x-text="item.room_name"></h5>
                                                        <span class="text-sm font-bold text-slate-900"
                                                            x-text="'₱' + Number(item.room_price).toLocaleString()"></span>
                                                    </div>
                                ...
                                <div class="bg-white border-t border-slate-100 p-5 rounded-b-2xl"
                                    ...
                                    <div class="flex justify-between items-end mb-4">
                                        ...
                                        <span class="text-2xl font-bold text-slate-800 tracking-tight"
                                            x-text="'₱' + Number(totalPrice).toLocaleString()"></span>
                                    </div>
                                    <div class="grid grid-cols-1 gap-3">
                                        <a href="{{ route('cart.index') }}"
                                            class="px-4 py-3 rounded-xl bg-teal-600 text-white text-sm font-bold hover:bg-teal-700 transition shadow-md shadow-teal-600/20 text-center">
                                            View Cart & Checkout
                                        </a>
                                    </div>
                                </div>
                        ...
                        <div id="nav-menu"
                            class="hidden lg:hidden absolute top-full left-0 w-full bg-white border-b border-slate-100 shadow-xl overflow-hidden transition-all duration-300 origin-top">
                            <div class="p-6 space-y-6">
                                <div class="flex flex-col space-y-4 font-medium text-slate-600 text-lg">
                                    <a href="{{ url('/') }}" class="flex items-center justify-between group">
                                        Home <i
                                            class="fa-solid fa-chevron-right text-xs opacity-0 group-hover:opacity-100 text-teal-500 transition-opacity"></i>
                                    </a>
                                ...
                                <div class="pt-6 border-t border-slate-100">
                                    <a href="{{ route('user.rooms.index') }}"
                                        class="block w-full py-3 bg-teal-900 text-white text-center rounded-lg font-medium shadow-lg shadow-teal-900/20 text-base">
                                        Book Your Stay
                                    </a>
                                </div>

                                <div class="flex justify-center gap-6 pt-2">
                                    <a href="#" class="text-slate-400 hover:text-teal-600 text-2xl"><i
...
<script>
    ...
    function cartDropdown() {
        return {
            open: false,
            cartItems: {}, // Object from PHP session
            items: [], // Array for length checking
            totalCount: 0,
            totalPrice: 0,
            ...
            fetchCart() {
                fetch('{{ route('cart.details') }}')
                    ...
                    .then(data => {
                        ...
                        this.items = Object.values(data.cart); // Convert object to array for length check
